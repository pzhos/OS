...
void processMessages(const std::string& file_name, HANDLE hStartEvent, HANDLE hInputReadySemaphore, HANDLE hOutputReadySemaphore, HANDLE hMutex) {
    std::fstream file;
    cout << "Write 1 to write secret message;\nWrite 0 to end the process\n";
    std::string key;
    cin >> key;
    ...
    while (true) {
        if (key == "1") {
            ...
            if (ReleaseSemaphore(hInputReadySemaphore, 1, NULL) != 1) {
                cout << "you have exceeded numbers of secret messages!";
                ReleaseSemaphore(hOutputReadySemaphore, 1, NULL);
        ...
        if (key != "0" && key != "1") {
            cout << "\nIncorrect value!\nWrite 1 to write secret message; \nWrite 0 to end the process\n";
            cin >> key;
        }
